...
class WhatsAppChatInboundService:
        ...
        for entry in update.entry:
            ...
            for change in entry.changes:
                ...
                if not value.messages:
                    ...
                    continue
                message_contexts.extend((message, value) for message in value.messages)
        if not message_contexts:
            log.w(f"  No messages found in update: {update}")
            return []
        sorted_contexts = sorted(message_contexts, key = lambda context: int(context[0].timestamp))
        return [self.ingest_message(message, value) for message, value in sorted_contexts]

    def ingest_message(self, message: WhatsAppMessage, value: Value) -> IngestedChatMessage:
        log.t(f"Ingesting WhatsApp message: {message}")
        mapper = self.__di.whatsapp_domain_mapper

        # let's store the chat first, it's the basis ('save' deduplicates)
        stored_chat = self.__di.chat_config_repo.save(mapper.map_chat(message, value))

        # then we store the message author
        mapped_author = mapper.map_author(message, value)
        is_author_the_agent = bool(mapped_author and is_the_agent(mapped_author, ChatConfigDB.ChatType.whatsapp))
        ...
        if is_author_the_agent:
            stored_attachments = self.__di.chat_attachment_repo.get_all_by_message(stored_chat.chat_id, message.id)
        else:
            if mapped_attachments := mapper.map_attachments(message):
                if stored_author is None:
                    raise InternalError("WhatsApp attachment cannot be stored without a message author", PLATFORM_MAPPING_FAILED)
                stored_attachments = [
    ...
    def store_attachment(
        ...
        if not content:
            raise ExternalServiceError(f"Couldn't download WhatsApp media '{attachment.external_id}'", MEDIA_DOWNLOAD_FAILED)
